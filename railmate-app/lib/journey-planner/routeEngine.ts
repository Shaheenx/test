 ...
 *
 * Data sourced from official BR Western Zone (text-layer PDF) and Eastern
 * Zone (scanned-image PDF) timetables — see PROVENANCE_AUDIT_FINAL.md for
 * the full per-train breakdown. ~97% of trains have PDF-traced stop data;
 * mail/commuter entries have origin+destination only (see confidenceGroup
 * on each TrainMeta). This is still a journey-planning aid, not a live
 * timetable — schedules change; always verify at eticket.railway.gov.bd
 * before travel.
 */
...
export interface TrainMeta {
  number: number;
  name: string;
  type: 'intercity' | 'commuter' | 'special' | 'mail';
...
const CONF_GROUP_SCORES: Record<ConfidenceGroup, number> = {
  g1: 90,  // VERIFIED_PDF / VERIFIED_PDF_STATION — western zone text-layer PDF, direct read
  g2: 75,  // VERIFIED_IMG — eastern zone scanned-image PDF, vision-read
  g3: 40,  // PDF_SUMMARY_COMMUTER(_MAIL) — official summary table, origin+destination only
  g4: 10,  // No verified stop data found for this train
  g5: 10,  // (unused by current data; kept for type compatibility)
};
